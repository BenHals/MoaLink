...
def make_moa_command(stream_string, learner, concept_limit, evaluator, length, report_length, directory, is_bat = True):
    return f'java -cp {"%" if is_bat else "$"}1\moa.jar -javaagent:{"%" if is_bat else "$"}1\sizeofag-1.0.4.jar moa.DoTask "{MOA_EVALUATORS[evaluator]} -l {get_learner_string(learner, concept_limit)} -s {stream_string} -e (BenPerformanceEvaluator -w {report_length})-i {length} -f {50}" > "{directory}{os.sep}{baseline}.csv"'
